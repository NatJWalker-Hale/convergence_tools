...
"""
originally phylo3.py, taken from Y. Yang and S. Smith, adh_2016
"""
...
class Node:
    ...
    def number_tree(self):
        # modifies in place
        c = 0
        for n in self.iternodes(order=1):
            if n.parent is None:
                continue
            else:
                n.number = c
...
##         return v
    ...
    def get_ancestors(self, include_self=False, until=None):
        if include_self:
            yield self
        node = self
        while node.parent is not None and node != until:
            node = node.parent
            yield node
    ...
    def get_descendants(self, include_self=False):
        if include_self:
            yield self
        for child in self.children:
            yield child.get_descendants(True)
